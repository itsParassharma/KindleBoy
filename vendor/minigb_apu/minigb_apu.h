...
/* KindleBoy local patch: default to the S16 native format when the caller
 * hasn't picked one, so every TU agrees on audio_sample_t == int16_t without
 * needing a -D on each compile line. Remove if you set the format elsewhere. */
